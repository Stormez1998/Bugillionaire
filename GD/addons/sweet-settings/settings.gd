...
# when nothing is focused, these actions will pull focus into the panel
const NAV_ACTIONS: Array[StringName] = [&"ui_up", &"ui_down", &"ui_left", &"ui_right", &"ui_accept"]

@onready var body = $Body
@onready var tab_bar = $Header/TabBar
@onready var exit_button = $Header/ExitButton
...
func _ready() -> void:
	...
	SweetSettings.input_device_changed.connect(_on_input_device_changed)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	for action in NAV_ACTIONS:
		if not event.is_action_pressed(action):
			continue
		var first := _first_focusable(_panels[_active_tab])
		if first != null:
			first.grab_focus()
...
func _on_input_device_changed(_family: String) -> void:
...
func _refresh_input_devices() -> void:
	for section_id in _section_components:
		for control in _section_components[section_id]:
			if control.has_method("refresh_input_device"):
				control.refresh_input_device()
	for key in _rows:
		var row = _rows[key]
		if row.has_method("refresh_input_device"):
			row.refresh_input_device()

func _refresh_keybind_conflicts() -> void:
	var conflicts := SweetSettings.get_keybind_conflicts()
	var labels: Dictionary = _keybind_labels()
	for key in _rows:
		var row = _rows[key]
		if row.has_method("refresh_keybind_conflicts"):
			row.refresh_keybind_conflicts(conflicts, labels)
...
func _keybind_labels() -> Dictionary:
	var labels: Dictionary = {}
	if SweetSettings.schema == null:
		return labels
	for key in SweetSettings.schema.keys():
		labels[str(key)] = _setting_display_label(str(key))
	return labels

func _setting_display_label(key: String) -> String:
