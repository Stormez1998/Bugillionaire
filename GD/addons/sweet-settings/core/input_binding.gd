...
static func _erase_bindable_events(action: String) -> void:
	var stale: Array[InputEvent] = []
	for event in InputMap.action_get_events(action):
		if not column_for_event(event).is_empty():
			stale.append(event)
...
static func display_keycode(event: InputEventKey) -> Key:
	if event == null:
		return KEY_NONE
