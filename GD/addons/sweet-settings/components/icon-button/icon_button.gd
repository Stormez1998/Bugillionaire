...
func _ensure_nodes() -> void:
	...
	_button = $Button
	_button_focus_mode = _button.focus_mode
...
func set_disabled(disabled: bool) -> void:
	...
	# Godot's focus navigation ignores 'disabled', so clear focus_mode too or a
	# d-pad walks onto a disabled keybind slot. Covers every slot and the + button.
	_button.focus_mode = Control.FOCUS_NONE if disabled else _button_focus_mode
	if disabled and _button.has_focus():
