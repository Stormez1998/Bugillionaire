...
# Which column this slot belongs to. Set once at creation; decides which
# "press anything" glyph the slot shows while listening.
var _column: String = InputBinding.KEYBOARD
...
# API
#================================================================================#
# Called once by KeybindPairs when the slot is created. A slot never changes
# column, so this is set rather than passed through set_listening on every call.
func set_column(column: String) -> void:
	_column = column
...
# INTERNAL
#================================================================================#
# Mirrors the column: a blank key means "press any key", a blank button under a
# finger means "press any button". Prompting for the wrong device is misleading,
# because the listeners discard input from the other column outright.
func _waiting_texture() -> Texture2D:
	if _column == InputBinding.CONTROLLER:
		return IconPaths.load_texture(IconPaths.join("generic", "generic_button_finger.svg"))
