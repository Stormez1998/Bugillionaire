...
func set_conflicts(conflicts: Dictionary, labels: Dictionary = {}) -> void:
	_ensure_nodes()
	_pairs.set_conflicts(conflicts if conflicts != null else {}, _setting_key, labels)

func refresh_input_device() -> void:
	_ensure_nodes()
	_pairs.refresh_input_device()
...
func _input(event: InputEvent) -> void:
	...
	# Listening is modal. Anything not passed through above is swallowed, including
	# events for the column we are not listening on — otherwise they reach the GUI
	# and move focus off the slot while _listening_index stays pinned to it.
	get_viewport().set_input_as_handled()

# The clear badge is the only thing allowed to see input while listening.
func _passes_through(event: InputEvent) -> bool:
	# Motion keeps hover and tooltips alive on the badge.
	if event is InputEventMouseMotion:
		return true

	if event is InputEventMouseButton:
		var mouse: InputEventMouseButton = event as InputEventMouseButton
		return _pairs.is_clear_badge_at(_listening_column, _listening_index, mouse.global_position)
