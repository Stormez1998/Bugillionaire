...
func set_conflicts(conflicts: Dictionary, setting_key: String, labels: Dictionary = {}) -> void:
	_conflicts = conflicts if conflicts != null else {}
	_setting_key = setting_key
	_labels = labels if labels != null else {}
	_refresh_slots()
...
func _refresh_slot(entry: Dictionary) -> void:
	...
	if not _conflicts.has(fingerprint):
		slot.set_conflicted(false)
		return

	slot.set_conflicted(true, _conflict_tooltip(_conflict_keys_for(fingerprint)))

func _conflict_keys_for(fingerprint: String) -> Array:
	var keys: Array = []
...
func _fallback_label(event: InputEvent) -> String:
	...
	if event is InputEventKey:
		var code := InputBinding.display_keycode(event as InputEventKey)
